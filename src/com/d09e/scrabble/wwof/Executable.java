...
public class Executable {
	...
	public static void main(String[] args) throws FileNotFoundException,
		...
		while (true) {
			...
			System.out.println(currentGame.getRack().get(0) + " "
					+ currentGame.getTempRack().get(0));
			while (true) {/*   */
				System.out.println(currentGame.getRack().get(0) + " "
						+ currentGame.getTempRack().get(0));
				System.out
						.println("Which tile would you like to place on the board? Enter 0 to finish.");
				System.out
						.println("Enter the NUMBER cooresponding to the tile!");

				for (int count = 0; count < currentGame.getTempRack().size(); count++) {
					if (currentGame.getTempRack().get(count) != '!')
						System.out.println((count + 1) + ":"
								+ currentGame.getTempRack().get(count));
				}

				if (counter == 0 && firstTurn != 0)
					System.out
							.println("8: discard your rack for a new rack(lose a turn)");

				System.out
						.println("\nNote to user: When you choose the first tile, you will be able to place it at a particular spot on the board.");
				System.out
						.println("All subsequent tiles that you place after the first tile will be placed automatically according to the direction you choose.");
				...
				if (counter == 0) {/*   */
					...
					if (tile != 0) {
						...
						if (firstTurn != 0) {
							...
							System.out
									.println("Where would you like to place this tile?");
							scan = new Scanner(System.in);
							colChar = scan.next();
							...
							System.out
									.println("Would you like to go horizontal or vertical?");
						...
						if (dir == 0)
							System.out.println(currentGame.getEnteredWord(
									firstRow, firstCol, dir));
						else if (dir == 1)
							System.out.println(currentGame.getEnteredWord(
									firstCol, firstRow, dir));
						break;
					...
					if (tile != 0) {
						System.out.println(currentGame.getRack().get(0) + " "
								+ currentGame.getTempRack().get(0));
						if (dir == 0)
							currentGame.placeTile(tile - 1, row, col + counter,
									dir);
						else if (dir == 1)
							currentGame.placeTile(tile - 1, row - counter, col,
									dir);
						...
						if (dir == 0)
							entered = currentGame.getEnteredWord(firstCol,
									firstRow, dir);
						else if (dir == 1)
							entered = currentGame.getEnteredWord(firstRow,
									firstCol, dir);
						...
						if (currentGame.checkBoard() == false
								|| currentGame.checkLength(entered, counter) == false) {/*   */
							System.out.println(currentGame.getRack().get(0)
									+ " " + currentGame.getTempRack().get(0));
							System.out.println();
							System.out
									.println("The tiles you have placed are invalid! Please try again.");
							System.out.println();
							counter = 0;
